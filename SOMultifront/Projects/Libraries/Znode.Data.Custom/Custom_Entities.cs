...
{
    public partial class Custom_Entities : IDBContext
    {
        #region Variables
        ...
        public void SetDataIntoEntity(DbEntityEntry dbEntry, int loginUserAccountId, int createdBy = 0, int modifiedBy = 0)
        {
            if (Equals(dbEntry.State, EntityState.Added))
            {
                ...
                dbEntry.Entity.SetPropertyValue(CreatedDate, DateTime.UtcNow.ToSiteConfigEntityUtcDateTime());
                dbEntry.Entity.SetPropertyValue(ModifiedDate, DateTime.UtcNow.ToSiteConfigEntityUtcDateTime());
            }
            else if (Equals(dbEntry.State, EntityState.Modified))
            {
                ...
                if (loginUserAccountId > 0)
                {
                    dbEntry.Entity.SetPropertyValue(ModifiedBy, loginUserAccountId);
                }
                dbEntry.Entity.SetPropertyValue(ModifiedDate, DateTime.UtcNow.ToSiteConfigEntityUtcDateTime());
            }
        }
        ...
        private void ReplaceCorruptEntity()
        {
            ...
            //Replace corrupt entites with the correct exisiting values, causing the EDMX module error.
