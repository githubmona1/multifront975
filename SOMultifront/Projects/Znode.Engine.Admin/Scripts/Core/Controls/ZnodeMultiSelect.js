...
var MultiSelectDDL = /** @class */ (function (_super) {
    ...
    MultiSelectDDL.prototype.BindSearch = function () {
        $('.ms-search input').on('keyup', function () {
            ...
            var instance = $(this).closest('.dropdown');
            var optionsList = instance.find('ul');
            var search = $(this).val();
            optionsList.find('li:not(.optgroup)').each(function () {
                var optText = $(this).text();
                var hidecolumn = $(this).hasClass("hidecolumn") ? true : false;
                // show option if string exists
                if (optText.toLowerCase().indexOf(search.toLowerCase()) > -1 && !hidecolumn) {
                    $(this).show();
                }
    ...
    MultiSelectDDL.prototype.GetRecord = function (IsAjax, Controller, Action, ParentMenuIds, SuccessCallBack, flag, target, entityId) {
        if (entityId === void 0) { entityId = 0; }
        if (IsAjax) {
            var _data = entityId > 0 ? { id: ParentMenuIds.toString(), flag: flag, entityId: entityId } :
                { id: ParentMenuIds.toString(), flag: flag };
            $.ajax({
    ...
    MultiSelectDDL.prototype.CheckBoxChecked = function (control, hiddenItem) {
        ...
        var _EntityId = $(control).data("entity-id");
        if (_IsSubmit.toLowerCase() === "false") {
            ...
            this.GetRecord(true, _Controller, _Action, MultiSelectDDL.prototype._Itemdata, _SuccessCallBack, _flag, control, _EntityId);
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (typeof tabId != 'undefined') {
                    ...
                    if (data.HasNoError)
                        ZnodeNotification.prototype.DisplayNotificationMessagesHelper(data.Message, 'success', true, fadeOutTime);
                    else
                        ZnodeNotification.prototype.DisplayNotificationMessagesHelper(data.Message, 'error', true, fadeOutTime);
                }
                else {
                    location.reload();
                }
