...
var DynamicGrid = /** @class */ (function (_super) {
    ...
    DynamicGrid.prototype.SetColumnList = function (columnList) {
        ...
        $(document).on("change", ".header-check-all", function () {
            ...
            if (this.checked) {
                ...
                $(this).closest('#grid').find('tr').find('td:eq(' + index + ') input[type=checkbox]:enabled').each(function () {
                    if ($.inArray($(this).attr("id"), CheckBoxCollection) == -1)
    ...
    DynamicGrid.prototype.OnFilterColumnChange = function (e) {
    ...
    DynamicGrid.prototype.GetFilterColumnList = function (columnId) {
        var filterColumnObj = {};
    ...
    DynamicGrid.prototype.GetInputField = function (filterColumnObj, dateFormat) {
        ...
        if (filterColumnObj['dataType']) {
            if (filterColumnObj['dataType'].toLowerCase() == "boolean") {
                ...
                return ('<input id="filtercolumn" class="filtercolumn datepicker" type="text" style="float:left;width:150px;"' +
    ...
    DynamicGrid.prototype.RemoveFilterButton = function (control, filterName) {
        if (filterName != null) {
    ...
    DynamicGrid.prototype.RemoveFilterText = function (control, filterName) {
        if (filterName != null) {
            $(control).parentsUntil('#filterComponent').find('[name="' + filterName + '"]').val("");
        }
    ...
    DynamicGrid.prototype.GetCurrentContext = function (selector) {
        return $(selector).parentsUntil('#filterComponent');
    };
    DynamicGrid.prototype.AddFilterButton = function (control) {
    ...
    DynamicGrid.prototype.EnableDisableAddFilterButton = function (selector, isDisabled) {
    ...
    DynamicGrid.prototype.RefreshGridNoNotification = function (control) {
    ...
    DynamicGrid.prototype.RefreshGridOnNewView = function (control, response) {
        $(control).closest('section').find("#refreshGrid").click();
    ...
    DynamicGrid.prototype.GetMultipleSelectedIds = function (target) {
        ...
        var result = [];
        $.each(CheckBoxCollection, function (i, e) {
            if ($.inArray(e.slice(e.indexOf("_") + 1), result) == -1)
                result.push(e.slice(e.indexOf("_") + 1));
        });
        return result.join();
    };
    ...
    DynamicGrid.prototype.CreateNewView = function (control) {
        var _isPublic;
    ...
    DynamicGrid.prototype.SetDefaultView = function (control) {
    ...
    DynamicGrid.prototype.ChangeViewVisibility = function (control) {
        var _isDefault = ($(control).attr('data-isDefault') == 'True' ? true : false);
        var _isPublic = !($(control).attr('data-isPublic') == 'True' ? true : false);
        this.EditView(control, _isDefault, _isPublic);
    };
    DynamicGrid.prototype.EditView = function (control, _isDefault, _isPublic) {
        var _targetid = $(control).data('targetid');
        var _itemId = $(control).data('id');
    ...
    DynamicGrid.prototype.DeleteView = function (control) {
        var _targetid = $(control).data('targetid');
        var _itemId = $(control).data('id');
    ...
    DynamicGrid.prototype.GetView = function (control) {
        var _itemId = $(control).data('id');
    ...
    DynamicGrid.prototype.GetMultipleValuesOfGridColumn = function (columnName) {
        var column = [];
        var index = 0;
    ...
    DynamicGrid.prototype.GetIndexOfColumn = function (column, columnName) {
    ...
    DynamicGrid.prototype.GetColumnValue = function (index) {
        var value = "";
    ...
    DynamicGrid.prototype.ClearAllFilter = function (control) {
    ...
    DynamicGrid.prototype.ClearGlobalFilter = function (control, ColumnName) {
        if ($(control).closest("form").find('input[name="' + ColumnName + '"]').length > 0) {
    ...
    DynamicGrid.prototype.SetCheckboxValue = function (control) {
    ...
    DynamicGrid.prototype.RemoveModalOverlay = function () {
