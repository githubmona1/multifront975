...
var QuickOrderPad = /** @class */ (function (_super) {
    ...
    QuickOrderPad.prototype.OnQuantityChange = function (control) {
    ...
    QuickOrderPad.prototype.MapProductDetails = function (control, res) {
    ...
    QuickOrderPad.prototype.SetAutoCompleteItemProperties = function (control, productId) {
    ...
    QuickOrderPad.prototype.DownloadQuickOrderTemplate = function (fileType) {
        window.location.href = window.location.protocol + "//" + window.location.host + "/Product/DownloadQuickOrderTemplate?fileType=" + fileType;
    };
    QuickOrderPad.prototype.ValidateQuickOrderItems = function () {
        ...
        $('#quick-order-pad-content [data-autocomplete-url]').each(function () {
            if (this.id.length > 0 && !isNaN(parseInt($(this).attr("data_qo_product_id"))) && $(this).attr("data_qo_product_id") != "" && $(this).attr("data_qo_product_id") != "0") {
                ...
                var result = QuickOrderPad.prototype.ValidateAutoCompleteItemProperties(this);
    ...
    QuickOrderPad.prototype.IsValidProductSKU = function (control) {
    ...
    QuickOrderPad.prototype.CheckUpdateSKU = function (control, e) {
    ...
    QuickOrderPad.prototype.IsValidateQuantity = function () {
        var textBoxIdsArray = ($("*[id*='txtQuickOrderPadQuantity_']")).map(function () {
            return this.id;
        }).get();
        var textBoxSplittedIdsArray = [];
        for (var textBoxIdIndex = 0; textBoxIdIndex < textBoxIdsArray.length; textBoxIdIndex++) {
            var textBoxSplittedId = (textBoxIdsArray[textBoxIdIndex].split("_")[1]);
            textBoxSplittedIdsArray.push(textBoxSplittedId);
        }
        for (var textBoxSplittedIdIndex = 0; textBoxSplittedIdIndex < textBoxSplittedIdsArray.length; textBoxSplittedIdIndex++) {
    ...
    QuickOrderPad.prototype.IsValidProductSKUQuick = function (control) {
        if ($(control).val() != $(control).attr("data_qo_sku") && ($(control).val() != "")) {
            $('p[for="' + control.id + '"]').html(ZnodeBase.prototype.getResourceByKeyName("ErrorValidSKU"));
            $('#btnQuickOrderClearAll').prop('disabled', false);
            return false;
    ...
    QuickOrderPad.prototype.AddProductsToQuickOrder = function () {
    ...
    QuickOrderPad.prototype.UploadQuickOrderFile = function (files) {
    ...
    QuickOrderPad.prototype.LoadBlankQuickGridRow = function (index) {
        var rowIndex;
        var i;
        for (i = 1; i <= index; i++) {
            rowIndex = i + 99;
    ...
    QuickOrderPad.prototype.DisplayQuickOrderNotification = function (response) {
    ...
    QuickOrderPad.prototype.GetFirstOrderRowId = function () {
        var index = 1;
        var flag = true;
        $('#quick-order-pad-content [data-autocomplete-url]').each(function () {
    ...
    QuickOrderPad.prototype.CheckDuplicateSKUs = function () {
        var listOfSKUs = [];
        $('#quick-order-pad-content [data-autocomplete-url]').each(function () {
    ...
    QuickOrderPad.prototype.LimitQuickOrderUptoFiftyRecords = function (res) {
        var count = 0;
        if ($('.form-group').length > 51) {
            for (var i = 51; i < $(".form-group").length; i++) {
                $(".form-group")[i].remove();
                count++;
            }
    ...
    QuickOrderPad.prototype.MergeDuplicateSKUs = function () {
        var listOfSKUs = [];
        $('#quick-order-pad-content [data-autocomplete-url]').each(function () {
            if ($(this).attr("data_qo_sku") != "" && this.id.length > 0) {
                listOfSKUs.push($(this).attr("data_qo_sku"));
    ...
    QuickOrderPad.prototype.ValidateAutoCompleteItemProperties = function (control) {
        ...
        if (control.id.length > 0 && !isNaN(parseInt($(control).attr("data_qo_product_id"))) && $(control).attr("data_qo_product_id") != "" && $(control).attr("data_qo_product_id") != "0") {
            ...
            var groupProductSKUs = $(control).attr("data_qo_group_product_sku");
            var configurableProductSKUs = $(control).attr("data_qo_configurable_product_sku");
        ...
        return { isSuccess: isSuccess, showAddToCart: showAddToCart };
    };
    ...
    QuickOrderPad.prototype.ShowRemoveItemBoxForNewRow = function (Quantitypadidnewrow) {
    ...
    QuickOrderPad.prototype.MergeDuplicateCartItems = function (cartItems) {
        var listOfSKUs = [];
    ...
    QuickOrderPad.prototype.AddMultipleOrdersToCart = function () {
        ...
        $('#btnQuickOrderPad').on("click", function () {
            ...
            $('#quick-order-pad-content [data-autocomplete-url]').each(function () {
                ...
                index++;
                var lastIndex = this.id.charAt(this.id.length - 1);
                $('p[for="' + this.id + '"]').html("");
                if (this.id.length > 0 && !isNaN(parseInt($(this).attr("data_qo_product_id"))) && $(this).attr("data_qo_product_id") != "" && $(this).attr("data_qo_product_id") != "0") {
                    var groupProductQty = void 0;
